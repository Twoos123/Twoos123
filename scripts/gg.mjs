// gg.svg: the footer, pointing to the portfolio.
...
<animate attributeName="x1" values="-1;1" dur="4s" repeatCount="indefinite"/><animate attributeName="x2" values="0;2" dur="4s" repeatCount="indefinite"/>
  ...
  const style = `
.gg { font: 800 48px ${HUD}; letter-spacing: 2px; }
...
<text x="${W / 2}" y="80" text-anchor="middle" class="gg" fill="url(#gg)">${esc(config.site)}</text>
<text x="${W / 2}" y="114" text-anchor="middle" class="cta">Projects, case studies, experience and resume</text>
<text x="${W / 2}" y="144" text-anchor="middle" class="sub">EVERYTHING ABOVE IS UPDATED DAILY FROM MY GITHUB</text>`;
  return svg(W, H, `Visit ${config.site} for the full portfolio`, defs, style, body);
}
